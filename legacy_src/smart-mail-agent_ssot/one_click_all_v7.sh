...
p=int(os.getenv("SMA_SMTP_PORT") or "587")
...
try:
    ...
    if tls=="starttls": srv.starttls(context=ssl.create_default_context()); srv.ehlo()
    if u and pw: srv.login(u,pw)
    srv.noop(); srv.quit()
    print("[OK] SMTP verified")
except Exception as e:
    print(f"[FAIL] SMTP verify failed: {e}"); sys.exit(2)
PY

say "verify: run dir & artifacts health"
python - <<'PY'
import pathlib, sqlite3
roots=sorted(pathlib.Path("reports_auto/e2e_mail").glob("*"))
if not roots:
    print("[WARN] no run dir found under reports_auto/e2e_mail; you may need to generate a batch.")
else:
    rd=str(roots[-1]); ts=pathlib.Path(rd).name
    ...
    if db.exists():
        ...
        cnt=cur.execute("SELECT COUNT(1) FROM actions WHERE run_ts=?",(ts,)).fetchone()[0]
        print(f"[DB] actions rows for run {ts}: {cnt}")
        con.close()
    else:
...
# 寄送（含附件；成功後會把 DB 的 SendEmail.payload_ref 改寫為對應 .eml）
...
sqlite3 db/sma.sqlite "
  SELECT case_id, COALESCE(action_type,action) AS type, status, payload_ref
  FROM actions
