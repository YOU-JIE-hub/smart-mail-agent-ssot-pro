#!/usr/bin/env bash
# 入倉 spam/intent → KIE 權重優先 artifacts_inbox/kie/kie/model.safetensors → 組評測集(多來源去重+容錯)
...
LOG="$ERR_DIR/ONECLICK_v6_${TS}.log"
SUMMARY="$STATUS_DIR/ONECLICK_v6_${TS}.md"
...
try_stage(){ local name="$1" dst="$2"; stage_one "$INBOX/$name" "$dst"; stage_one "/mnt/data/$name" "$dst"; }
# Spam
...
if p.exists():
    j=json.loads(p.read_text("utf-8")); thr=j.get("spam") if isinstance(j.get("spam"),(int,float)) else j.get("threshold",0.44)
    p.write_text(json.dumps({"spam": float(thr)},ensure_ascii=False,indent=2),"utf-8"); print("[OK] spam thresholds ->", float(thr))
else:
    p.write_text(json.dumps({"spam":0.44},indent=2,ensure_ascii=False),"utf-8"); print("[OK] spam thresholds -> default 0.44")
...
if ir.exists():
    j=json.loads(ir.read_text("utf-8")); kw=j.get("keywords") or j
    if isinstance(kw,dict):
        rx={k:"("+"|".join(map(re.escape,(v or [])))+")" if not isinstance(v,str) else v for k,v in kw.items()}
        ...
        print("[WARN] intent_rules.json 非預期格式，略過編譯")
else:
...
root=pathlib.Path(".")
def sha(s): return hashlib.sha1((s or "").strip().encode("utf-8")).hexdigest()
...
def read_jsonl_relaxed(p):
    ok=bad=0; data=[]
    try:
        for i,ln in enumerate(pathlib.Path(p).read_text("utf-8",errors="ignore").splitlines(),1):
            if not ln.strip(): continue
            try:
                data.append(json.loads(ln)); ok+=1
            except Exception:
                bad+=1
        return data, ok, bad
...
spam_sources = [
    ...
    pathlib.Path("/mnt/data/external_realistic_test.clean.jsonl"),
...
rows=[]; used=[]
for p in spam_sources:
    dat,ok,bad = read_jsonl_relaxed(p)
    if ok+bad==0: continue
    for r in dat:
...
print(f"[SPAM] sources_used:")
for s,ok,bad in used: print(f" - {s} ok={ok} bad={bad}")
...
def norm_label(v):
    ...
    mp={"biz_quote":"報價","tech_support":"技術支援","complaint":"投訴","policy_qa":"規則詢問","profile_update":"資料異動","other":"其他",
        "報價":"報價","技術支援":"技術支援","投訴":"投訴","規則詢問":"規則詢問","資料異動":"資料異動","其他":"其他"}
    return mp.get(str(v).strip(), None)
intent_sources = [
    root/"data/intent_eval/dataset.jsonl",                      # 若已存在就會被重新寫（確保規格）
    pathlib.Path("/mnt/data/test_labeled.jsonl"),
    pathlib.Path("/mnt/data/test.jsonl"),
    pathlib.Path("/mnt/data/test.demo.jsonl"),
    pathlib.Path("/mnt/data/test.sample.jsonl"),
    pathlib.Path("/mnt/data/gold_for_train.jsonl"),
    pathlib.Path("/mnt/data/val.jsonl"),
    pathlib.Path("/home/youjie/projects/smart-mail-agent/data/intent_eval/dataset.jsonl"),
]
rows=[]; used=[]
for p in intent_sources:
    dat,ok,bad = read_jsonl_relaxed(p)
    if ok+bad==0: continue
    for r in dat:
        lab = norm_label(r.get("intent") or r.get("label"))
        if not lab: continue
        txt = text_of(r)
...
def rule_pick(text):
    ...
    return None
cm={}; labels=set([d.get("intent") for d in ds if d.get("intent")]); labels |= set(th.keys()); labels |= set(RX.keys())
for lab in labels: cm[lab]={"tp":0,"fp":0,"fn":0}
for d,p in zip(ds,pr):
    gold=d.get("intent"); text=d.get("text") or d.get("body") or d.get("subject") or ""
    pred=p.get("pred_intent"); conf=float(p.get("intent_conf") or 0.0)
    thr=th.get(pred, th.get("其他",0.4)); routed=pred
    if conf < thr: routed=rule_pick(text) or "其他"
    else:
        r=rule_pick(text)
