...
for _ in range(3):
    ...
    for lab in LABELS:
        ...
        for b in cands:
            ...
            if trial["micro"][2] > best_micro + 1e-6:
                best_micro = trial["micro"][2]; best_bias = trial_bias; best = trial; improved = True
    for lab, pool in [("投訴",[0.35,0.40,0.45,0.50]), ("報價",[0.35,0.40,0.45,0.50])]:
        for th in pool:
            ...
            if trial["micro"][2] > best_micro + 1e-6:
                best_micro = trial["micro"][2]; best_min = trial_min; best = trial; improved = True
...
md=[]
md.append("# Intent metrics (rules hotfix v9_fix)")
