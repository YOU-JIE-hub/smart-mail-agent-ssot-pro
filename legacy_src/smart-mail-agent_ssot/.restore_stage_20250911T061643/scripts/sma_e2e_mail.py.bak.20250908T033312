...
def main():
    ...
    for eml in src.glob("**/*.eml"):
        ...
        try:
            ...
            try:
                is_spam, sconf = __faq_res_is_spam_sconf
            except Exception:
                is_spam = getattr(__faq_res_is_spam_sconf, 'text', str(__faq_res_is_spam_sconf))
                sconf = getattr(__faq_res_is_spam_sconf, 'score', getattr(__faq_res_is_spam_sconf, 'confidence', None))

                    try:
                        __faq_res_action_type_apath = __faq_res_action_type_apath
                    except Exception:
