# --- sma hotfix: safe unpack helper ---
def _sma_unpack_answer(_res):
    try:
        a, b = _res
        return a, b
    except Exception:
        return (getattr(_res, 'text', str(_res)),
                getattr(_res, 'score', getattr(_res, 'confidence', None)))
# --- end hotfix ---
...
def ensure_db_and_migrate():
    ...
    ensure_column("intent_preds", "confidence", "confidence REAL")
    ensure_column("intent_preds", "created_at", "created_at TEXT")
    conn.commit()
    conn.close()
...
def main():
    ...
    for eml in src.glob("**/*.eml"):
        ...
        try:
            ...
            __faq_res_is_spam_sconf = spam_predict(text, spam_ctx)
            is_spam, sconf = _sma_unpack_answer(__faq_res_is_spam_sconf)
            if is_spam:
                summary["spam"] += 1
                ...
                if low:
                    ...
                    action_type, apath = _sma_unpack_answer(__faq_res_action_type_apath)
                elif label == "資料異動":
