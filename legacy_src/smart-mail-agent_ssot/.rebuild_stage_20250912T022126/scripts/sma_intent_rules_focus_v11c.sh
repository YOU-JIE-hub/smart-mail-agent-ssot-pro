...
RX = {
  ...
  "投訴":[
    ...
    r"賠償|補償|補救|違反\s*SLA|SLA\s*違反|影響上線|影響營運|高風險|造成損失|退款|退費|退貨|退刷|退單|道歉"
  ],
  "規則詢問":[
    # 針對政策/資安/合約/合規的典型說法（移除退款/退費，避免與投訴混淆）
    r"SLA|RPO|RTO|DPA|GDPR|HIPAA|ISO\s*27001|SOC\s*2|NIST|滲透測試|稽核|審計|合規|法遵|security\s*policy|資料保留|data\s*retention|刪除政策|隱私(?:權|政策)|privacy",
    r"續約|自動續約|終止|解約|合約|違約金|contract|assignment|notice|條款|terms|EULA|NDA"
  ],
  "資料異動":[
...
# 針對規則詢問的負向守門（出現報價語意則扣分）
NEG_PRICE = re.compile(r"報價|費用|價|總價|總額|NT\$|USD|US\$|quote|pricing|price", re.I)
# 針對投訴的強信號（退款/退費 + 抱怨語氣）
STRONG_COMPLAINT = re.compile(r"(退款|退費|退貨|退刷|退單).*(不滿|無法接受|太慢|延誤|造成損失|正式|投訴|申訴)", re.I)
    ...
    for lb, regs in CRX.items():
        for r in regs:
            for _ in r.finditer(txt):
                s[lb]+=1.0
    # 價格語意 ⇒ 規則詢問降分
    if NEG_PRICE.search(txt):
        s["規則詢問"]-=0.6
    # 退款/退費 + 負面語氣 ⇒ 投訴加分 & 規則詢問小扣
...
# 4) 小範圍搜尋（只調 投訴/規則詢問）
grid={
  "bias.投訴":[base["bias"].get("投訴",0.2)+d for d in (0.0,0.1,0.2)],
  "min.投訴":[max(0.1, base["min_keep"].get("投訴",0.35)+d) for d in (-0.10,-0.05,0.0)],
  "min.規則詢問":[base["min_keep"].get("規則詢問",0.55)+d for d in (-0.05,0.0,0.05)]
}
keys=list(grid.keys())
best=None
...
(Path(EVADIR)/"metrics_intent_rules_hotfix_v11c.md").write_text(md_metrics(res,"Intent metrics (rules hotfix v11c)"),"utf-8")
...
# 存 calib
out_calib=ROOT/"artifacts_prod/intent_rules_calib_v11c.json"
out_calib.write_text(json.dumps(calib,ensure_ascii=False,indent=2),"utf-8")
print("[OK] saved calib ->", out_calib.as_posix())
print(">>> Result =>", (Path(EVADIR)/"metrics_intent_rules_hotfix_v11c.md").as_posix())
PY

# 附掛到最新 ONECLICK
LATEST="$(ls -t reports_auto/status/ONECLICK_* 2>/dev/null | head -n1)"
if [ -n "${LATEST:-}" ]; then
  MET="$(ls -t reports_auto/eval/*/metrics_intent_rules_hotfix_v11c.md | head -n1)"
  { echo "## Intent metrics (rules hotfix v11c)"; sed -n '1,120p' "$MET"; } >> "$LATEST"
  tail -n 60 "$LATEST"
fi
