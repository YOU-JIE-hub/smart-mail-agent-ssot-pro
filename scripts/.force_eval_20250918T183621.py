...
def find_estimator(obj, seen=None):
  ...
  if isinstance(obj,dict):
    for k in ('pipeline','model','estimator','clf','classifier','final_estimator','base_estimator'):
...
def to_text_pipeline(obj):
  ...
  if est is not None:
    ...
    except Exception: return est
  return None
def record_shape(out_dir, tag, obj):
...
if ip and Path(ip).exists():
  raw=robust_load(ip); record_shape(OUTI,'intent_raw',raw)
  ...
  try:
    if pipe is None: raise RuntimeError('cannot_build_text_pipeline')
    pred=pipe.predict(X)
...
if sp and Path(sp).exists():
  raw=robust_load(sp); record_shape(OUTS,'spam_raw',raw)
  pipe=to_text_pipeline(raw)
  X=[]; y=[]
