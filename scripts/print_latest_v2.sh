...
OUT="$(readlink -f reports_auto/online/latest 2>/dev/null || ls -1dt reports_auto/online/* 2>/dev/null | head -n1 || true)"
if [ -z "$OUT" ] || [ ! -d "$OUT" ]; then
  echo "OUT: <none>"; echo "先跑：bash scripts/crashnet_v5.sh"; exit 0
fi
echo "OUT: $OUT"
...
PY
[ -s "$OUT/uvicorn.err" ] && { echo "--- tail uvicorn.err ---"; tail -n 60 "$OUT/uvicorn.err"; } || echo "<no uvicorn.err>"
[ -s "$OUT/run.log" ] && { echo "--- tail run.log ---"; tail -n 60 "$OUT/run.log"; }
