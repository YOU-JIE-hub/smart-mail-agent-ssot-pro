...
collect(){ bash scripts/sma_collect_logs.sh || true; }
on_err(){ ec=$?; echo "[DEEPDOCTOR ERR] ec=$ec" | tee -a "$RUN_LOG"; collect; if command -v explorer.exe >/dev/null 2>&1; then explorer.exe "$(wslpath -w "$ERR_DIR")" || true; fi; exit $ec; }
trap on_err ERR
