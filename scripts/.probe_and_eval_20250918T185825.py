import os,sys,hashlib,re,json,traceback,joblib,inspect
...
def snapshot(obj):
  ...
  return info
def is_pred_like(o):
  return any(hasattr(o,a) for a in('predict','predict_proba','decision_function'))
def looks_numeric_estimator(est):
  # 如果模型暴露 n_features_in_ 或拒絕字串，視為 numeric-only
  if hasattr(est,'n_features_in_'): return True
  return False
def build_text_pipeline(obj):
  # 1) dict: 嘗試 vectorizer+clf 常見鍵
  if isinstance(obj,dict):
    vec=obj.get('vectorizer') or obj.get('vect') or obj.get('tfidf') or obj.get('bow') or obj.get('preprocessor')
    clf=obj.get('clf') or obj.get('classifier') or obj.get('model') or obj.get('estimator') or obj.get('final_estimator')
  ...
  def dfs(x,seen):
    ...
    seen.add(id(x))
    if is_pred_like(x): cand=x; return
    if isinstance(x,dict):
      for v in x.values(): dfs(v,seen)
    elif isinstance(x,(list,tuple)):
      for v in x: dfs(v,seen)
  dfs(obj,set())
  if cand is None: return None,'not_found'
  # 3) 如果 estimator 看起來是 numeric-only，就別補 TF-IDF，交給 baseline
  if looks_numeric_estimator(cand): return None,'numeric_only_estimator'
  # 4) 嘗試補 TF-IDF 前端
  try: return make_pipeline(TfidfVectorizer(ngram_range=(1,2),min_df=1), cand), 'tfidf+estimator'
  except Exception: return cand,'raw_estimator'
# ===== Intent =====
