...
def as_id(s): import re; s=re.sub(r'[^A-Za-z0-9\u4e00-\u9fff._-]+','_', (s or "").strip()); return s[:80] or "mail"
mail=as_id(mail)
...
intent_map={"make_quote_pdf":"報價","create_ticket":"技術支援","escalation_suggestion":"投訴",
            "faq_reply_draft":"規則詢問","crm_update":"資料異動","human_handoff":"其他"}
...
def safe_name(s): import re; s=str(s or "").strip(); s=re.sub(r'[^A-Za-z0-9\u4e00-\u9fff._-]+','_',s); return s[:80] or "mail"
...
def mask_phone(s): import re; return re.sub(r'(?:(?:\+?886\-?)?0?9\d{2})[\-\s]?\d{3}[\-\s]?\d{3}', lambda m: m.group(0)[:4]+"-***-***", as_str(s))
def mask_email(s): import re; s=as_str(s); return re.sub(r'([A-Za-z0-9._%+-])([A-Za-z0-9._%+-]*)(@[^,\s>]+)', lambda m: (m.group(1)+"***"+m.group(3)), s)
def rag_search(query:str,k:int=3):
    roots=[Path("policies"), Path("data/faq")]; toks=set(re.findall(r"[A-Za-z0-9\u4e00-\u9fff]+", query or ""))
    hits=[]
    for r in roots:
        for p in r.rglob("*"):
...
for it in actions:
    ...
    try:
        if act=="make_quote_pdf":
            ...
            tax_rate=to_float(fields.get("tax_rate",0.05),0.05); subtotal=sum(i["qty"]*i["unit_price"] for i in items)
            tax=round(subtotal*tax_rate,2); total=subtotal+tax
...
<tfoot><tr><td colspan="3">小計</td><td style='text-align:right'>{subtotal:.2f}</td></tr><tr><td colspan="3">稅額({tax_rate*100:.0f}%)</td><td style='text-align:right'>{tax:.2f}</td></tr><tr><td colspan="3"><b>總額</b></td><td style='text-align:right'><b>{total:.2f}</b></td></tr></tfoot></table>"""
            ...
            try:
                ...
                if importlib.util.find_spec("weasyprint"):
                    ...
                    log.write("[INFO] weasyprint not installed, skip PDF\n")
            except Exception as e:
                log.write(f"[WARN] weasyprint failed: {type(e).__name__}: {e}\n")
        elif act=="create_ticket":
            p=outdir/"ticket.json"; p.write_text(json.dumps({
                "title":"[技術支援] 自動建單",
                "severity":str(fields.get("severity","minor")),
                "component":str(fields.get("component","api")),
                "has_logs":bool(fields.get("has_logs",False)),
