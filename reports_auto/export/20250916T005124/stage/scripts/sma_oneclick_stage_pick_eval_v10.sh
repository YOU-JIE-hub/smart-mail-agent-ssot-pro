#!/usr/bin/env bash
# v10: 入倉 spam/intent → KIE 權重優先 artifacts_inbox → 組評測集(多來源去重+容錯)
# → 跑 eval → 自算 Intent(門檻+規則) → 掛 KIE 附件 → 摘要（不讀 /mnt/data）
set -o pipefail
...
LOG="$ERR_DIR/ONECLICK_v10_${TS}.log"
SUMMARY="$STATUS_DIR/ONECLICK_v10_${TS}.md"
...
PREF_A="$INBOX/kie/kie/model.safetensors"
PREF_B="$INBOX/kie/model.safetensors"
if   [ -f "$PREF_A" ]; then cp -f "$PREF_A" "kie/kie/model.safetensors"; log "[APPLY] KIE -> kie/kie/model.safetensors (from artifacts_inbox/kie/kie)"
elif [ -f "$PREF_B" ]; then cp -f "$PREF_B" "kie/kie/model.safetensors"; log "[APPLY] KIE -> kie/kie/model.safetensors (from artifacts_inbox/kie)"
else
  CAND="$(find "$OLD" -type f -name model.safetensors -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR==1{print $2}')"
  if [ -n "$CAND" ] && [ -f "$CAND" ]; then cp -f "$CAND" "kie/kie/model.safetensors"; log "[APPLY] KIE -> kie/kie/model.safetensors (from $CAND)"
  else log "[WARN] 找不到 KIE 權重，略過覆蓋"; fi
fi
...
# 舊專案的固定名
op = Path("..")/"smart-mail-agent"
intent_paths += [str(op/"data/intent_eval/dataset.jsonl")]
intent_paths += [str(op/"reports_auto/intent/test.jsonl")]
intent_paths += [str(op/"reports_auto/intent/val.jsonl")]
intent_paths += [str(op/"reports_auto/intent/train.jsonl")]

# 也掃描專案根內部的通用檔名（如果你剛放進 artifacts_inbox/）
intent_paths += [str(p) for p in Path("artifacts_inbox").glob("test*.jsonl")]
intent_paths += [str(p) for p in Path("artifacts_inbox").glob("val*.jsonl")]
intent_paths += [str(p) for p in Path("artifacts_inbox").glob("train*.jsonl")]
intent_paths += [str(p) for p in Path("artifacts_inbox").glob("silver*.jsonl")]
intent_paths += [str(p) for p in Path("artifacts_inbox").glob("*labeled*.jsonl")]
intent_paths += [str(p) for p in Path("artifacts_inbox").glob("gold*.jsonl")]
intent_paths = list(dict.fromkeys(intent_paths))  # 去重路徑
...
for p in intent_paths:
    ...
    for ln in pth.read_text("utf-8",errors="ignore").splitlines():
        try:
            if not ln.strip() or ln.strip() in {"[","]","{","}"}: continue
            r=json.loads(ln)
            t=get_text(r)
            if not t: continue
            lab=None
...
collect_from "$INBOX"; collect_from "data/staged_project"; collect_from "$OLD"

{
  echo "# ONECLICK v10 (${TS})"
  echo "## Artifacts"
  echo '```'
