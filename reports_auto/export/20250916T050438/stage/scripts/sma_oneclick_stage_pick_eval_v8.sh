...
LOG="$ERR_DIR/ONECLICK_v8_${TS}.log"
SUMMARY="$STATUS_DIR/ONECLICK_v8_${TS}.md"
...
label_keys=("intent","label","category","class","y","target")
def map_intent(v):
    if v is None: return None
    s=str(v).strip()
    if s in zh_set: return s
    return en2zh.get(s.lower())
...
# 舊專案（若有）
intent_paths += [str(Path("..")/"smart-mail-agent/data/intent_eval/dataset.jsonl")]
...
for p in intent_paths:
    ...
    for ln in pth.read_text("utf-8",errors="ignore").splitlines():
        try:
            if not ln.strip(): continue
            r=json.loads(ln)
            t=(r.get("text") or r.get("body") or r.get("subject") or "").strip()
            if not t: continue
            lab=None
            for k in label_keys:
                if k in r:
