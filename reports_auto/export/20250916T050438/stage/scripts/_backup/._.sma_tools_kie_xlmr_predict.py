...
def _normalize_id2label(cfg):
    m = getattr(cfg, 'id2label', None)
    if isinstance(m, dict):
        z = {}
        for k,v in m.items():
            try: z[int(k)] = v
            except Exception: z[k] = v
        return z
    elif isinstance(m, (list, tuple)):
        return {i:s for i,s in enumerate(m)}
    else:
        return {i:str(i) for i in range(getattr(cfg,'num_labels',0))}
...
def main():
    ...
    with open(in_path,"r",encoding="utf-8") as fi, open(out_path,"w",encoding="utf-8") as fo:
        for ln in fi:
            ...
            labels=[id2label.get(int(i), id2label.get(str(i), str(i))) if isinstance(id2label,dict) else id2label[i] for i in pred]
