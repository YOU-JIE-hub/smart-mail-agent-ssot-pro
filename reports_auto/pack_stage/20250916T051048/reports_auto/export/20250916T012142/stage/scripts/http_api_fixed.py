...
# 讓 vendor/ 在 sys.path 前面（ZeroPad shim）
...
def _pick_predictor(obj):
    if hasattr(obj,"predict"):
        return obj
    if isinstance(obj, dict):
        for k in ("pipe","pipeline","estimator","model"):
            if k in obj and obj[k] is not None:
                try: return _pick_predictor(obj[k])
                except Exception: pass
    if isinstance(obj, (list, tuple)):
        for it in obj:
            try: return _pick_predictor(it)
            except Exception: pass
...
def load_pipe():
    ...
    _PIPE=_pick_predictor(obj)
    return _PIPE

# -------- Rule baseline / KIE / Plan / Act --------
ACTIONS={
    ...
    def do_POST(self):
        ...
        try:
            ...
            if p=="/act":
                ...
                for it in items:
                    (outdir/f"act_{it.get('action','noop')}_{it.get('mail_id','x')}.txt").write_text(
...
def run():
    ...
    print(f"[*] API(fixed) on :{port} (DRY_RUN={SMA_DRY_RUN})")
    srv.serve_forever()
