...
# 類別化閾值（放寬 env/sla）
MIN_SCORE = {"amount":0.50,"date_time":0.50,"env":0.30,"sla":0.30}
# 沒 score 的模型 span 視為這個分數
...
# 日期：YYYY/MM/DD、MM/DD、YYYY-MM、中文字（年/月/日）
RE_DATE = re.compile(
    r'(?:(\d{4})[\/\-.年](\d{1,2})[\/\-.月](\d{1,2})(?:日)?)'   # 2025/08/31
    r'|(?<!\d)(\d{1,2})[\/\-.](\d{1,2})(?!\d)'                  # 9/30
)

# 環境：加入更多別名/口語；避免單字「正式/測試」過廣，但保留「正式機/測試機/環境」等
ENV_WORDS = [
...
# SLA：支援全形字、分隔字元、同義詞（SLA/SLO/SLI/MTTR/MTBF/RTO/RPO…）
SLA_CORE = r'(?:[SＳ]\s*[/\-＿﹣–—_]?\s*[LＬ]\s*[/\-＿﹣–—_]?\s*[AＡ])'
RE_SLA = re.compile(
    rf'{SLA_CORE}'
    r'|SLO|SLI|MTTR|MTBF|RTO|RPO|uptime|availability|downtime|incident'
    r'|服務水準|服務等級|服務等級協議|服務水準協議|回應時間|回覆時間|回應時限|支援時段|支援時間|全年無休|24x7|7x24|7/24',
    re.I
)

def rule_spans(text):
    out=[]
    for m in RE_AMOUNT.finditer(text): out.append({"start":m.start(),"end":m.end(),"label":"amount","score":0.98,"src":"rule"})
    for m in RE_DATE.finditer(text):   out.append({"start":m.start(),"end":m.end(),"label":"date_time","score":0.98,"src":"rule"})
    for m in RE_ENV.finditer(text):    out.append({"start":m.start(),"end":m.end(),"label":"env","score":0.98,"src":"rule"})
    for m in RE_SLA.finditer(text):    out.append({"start":m.start(),"end":m.end(),"label":"sla","score":0.98,"src":"rule"})
    return out
...
# ===== 7) Export report =====
md=[]
md.append("# KIE span metrics (hybrid hotfix v3)")
...
# ===== 8) Dump hard examples: 未命中的金標（各標籤 <=20 範例）=====
